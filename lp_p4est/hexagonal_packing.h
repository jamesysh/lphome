...
//		return xmin+2.0*h_r*k;
		if(tag==0) return xmin+1.5*h_r+k*2.0*h_r+m_fInitialPerturbation*0.5*h_r*(2*((double)rand()/(double)RAND_MAX)-1); //odd-numbered rows
		else return  xmin+0.5*h_r+k*2.0*h_r+m_fInitialPerturbation*0.5*h_r*(2*((double)rand()/(double)RAND_MAX)-1); //even-numbered rows
	}
...
//		return ymin+2.0*h_r*j;
		return ymin+0.5*sqrt(3.0)*h_r+j*sqrt(3.0)*h_r+m_fInitialPerturbation*0.5*h_r*(2*((double)rand()/(double)RAND_MAX)-1);
	}
	void getInitialPerturbation(double ip){m_fInitialPerturbation=ip;}

private:
	double xmin, xmax, ymin, ymax, h_r;
	size_t m0, m1, n0_odd, n1_odd, n0_even, n1_even;	
	double m_fInitialPerturbation;	
	//HexagonalPacking2D() {}
...
public:
	 ...
	 */
	double computeX(int tag, size_t k) {
//		return xmin+h_r+2*k*h_r; 
		if(tag==0) return xmin+1.5*h_r+k*2.0*h_r+m_fInitialPerturbation*0.5*h_r*(2*((double)rand()/(double)RAND_MAX)-1); //(odd layers && odd rows) || (even layers && even rows)
		else return  xmin+0.5*h_r+k*2.0*h_r+m_fInitialPerturbation*0.5*h_r*(2*((double)rand()/(double)RAND_MAX)-1); // (odd layers && even rows) || (even layers && odd rows)
	}
...
//		return ymin+h_r+2*j*h_r;
		if(tag==0) return ymin+1.5*sqrt(3.0)/3.0*h_r+j*sqrt(3.0)*h_r+m_fInitialPerturbation*0.5*h_r*(2*((double)rand()/(double)RAND_MAX)-1);	//odd-numbered layers
		else return ymin+0.5*sqrt(3.0)/3.0*h_r+j*sqrt(3.0)*h_r+m_fInitialPerturbation*0.5*h_r*(2*((double)rand()/(double)RAND_MAX)-1); //even-numbered layers
	}
...
//		return zmin+h_r+2*i*h_r;
		return zmin+h_r+i*2.0*sqrt(6.0)/3.0*h_r+m_fInitialPerturbation*0.5*h_r*(2*((double)rand()/(double)RAND_MAX)-1);
//                return zmin+sqrt(6.0)/3.0*h_r+i*2.0*sqrt(6.0)/3.0*h_r+0.0*0.5*h_r*(2*((double)rand()/(double)RAND_MAX)-1);
	}
	void getInitialPerturbation(double ip){m_fInitialPerturbation=ip;}
